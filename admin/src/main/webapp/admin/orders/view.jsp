					...
					<c:forEach items="${order.orderItemList}" var="item" varStatus="vs">
						<tr style="FONT-WEIGHT: bold; FONT-SIZE: 12pt; HEIGHT: 25px; BACKGROUND-COLOR: #eeeeee">
							...
							<td width="8%" align="center">
								<img src="${pageContext.request.contextPath}${item.products.imgurl}" width="50" height="50">
							</td>
							<td align="center" width="18%">${item.products.id }</td>
							<td align="center" width="10%">${item.products.name }</td>
							<td align="center" width="10%">${item.products.price }</td>
							<td width="7%" align="center">${item.buynum }</td>
							<td width="7%" align="center">${item.products.category.name }</td>
							<td width="31%" align="center">${item.products.description}</td>
						</tr>
					</c:forEach>
				</table>
			</TD>
